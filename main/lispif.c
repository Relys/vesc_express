...
void lispif_init(void) {
	...
	memory_array =
		heap_caps_malloc(mem_size * sizeof(uint32_t), MALLOC_CAP_DMA);
	bitmap_array =
		heap_caps_malloc(bitmap_size * sizeof(uint32_t), MALLOC_CAP_DMA);
...
static void sym_it(const char *str) {
	...
	commands_printf_lisp(
		...
		sym_entry_flash ? "FLASH" : "L_MEM", str
	);
}
	...
	switch (packet_id) {
		...
		case COMM_LISP_GET_STATS: {
			...
			if (lbm_heap_state.gc_num > 0) {
				heap_use = 100.0
					* (float)(heap_size - lbm_heap_state.gc_last_free)
					/ (float)heap_size;
			}

			mem_use = 100.0
				* (float)(lbm_memory_num_words() - lbm_memory_num_free())
				...
				for (int i = 0; i < GLOBAL_ENV_ROOTS; i++) {
					...
					while (lbm_type_of(curr) == LBM_TYPE_CONS) {
						...
						if (lbm_type_of(lbm_car(key_val)) == LBM_TYPE_SYMBOL
							...
							const char *name = lbm_get_name_by_symbol(
								lbm_dec_sym(lbm_car(key_val))
							);
							...
							if (print_all
								|| ((name[0] == 'v' || name[0] == 'V')
									&& (name[1] == 't' || name[1] == 'T'))) {
								strcpy(
									(char *)(send_buffer_global + ind), name
								);
								ind += strlen(name) + 1;
		...
		case COMM_LISP_REPL_CMD: {
			...
			if (lisp_thd_running) {
				...
				if (len <= 1) {
					...
					commands_printf_lisp(
						":info\n"
						"  Print info about memory usage, allocated arrays and "
						"garbage collection"
					);
					commands_printf_lisp(
						":prof start\n"
					...
					commands_printf_lisp(" ");
					commands_printf_lisp(
						"Anything else will be evaluated as an expression in "
						"LBM."
					);
					commands_printf_lisp(" ");
					...
					commands_printf_lisp(
						"GC SP max: %u (size %u)\n",
						lbm_get_max_stack(&lbm_heap_state.gc_stack),
					...
					commands_printf_lisp(
						"Symbol table size: %u Bytes\n",
						lbm_get_symbol_table_size()
					);
					commands_printf_lisp(
						"Symbol table size flash: %u Bytes\n",
						lbm_get_symbol_table_size_flash()
					);
					commands_printf_lisp(
						"Symbol name size: %u Bytes\n",
						lbm_get_symbol_table_size_names()
					);
					commands_printf_lisp(
						"Symbol name size flash: %u Bytes\n",
					...
					commands_printf_lisp(
						"Erase Cnt Max Sector: %d\n", stats.erase_cnt_max
					);
					commands_printf_lisp(
						"Num sectors erased: %d\n", stats.erased_sector_num
					);
					...
					commands_printf_lisp(
						"Profiler stopped. Issue command ':prof report' for "
						"statistics\n"
					);
					...
					for (int i = 0; i < PROF_DATA_NUM; i++) {
						...
						commands_printf_lisp(
							"%d\t%s\t%u\t%.3f\t%.3f", prof_data[i].cid,
							prof_data[i].name, prof_data[i].count,
					...
					if (pause_eval(0, 1000)) {
						...
						for (int i = 0; i < GLOBAL_ENV_ROOTS; i++) {
							...
							while (lbm_type_of(curr) == LBM_TYPE_CONS) {
								lbm_print_value(
									output, sizeof(output), lbm_car(curr)
								);
								curr = lbm_cdr(curr);
					...
					commands_printf_lisp(
						lispif_restart(
							true, flash_helper_code_size(CODE_IND_LISP) > 0,
							true
						)
							? "Reset OK\n\n"
							: "Reset Failed\n\n"
					...
					if (pause_eval(30, 1000)) {
						...
						commands_printf_lisp(
							"%s",
							lbm_undefine(sym) ? "Cleared bindings"
											  : "No definition found"
						);
						lbm_continue_eval();
					}
					...
					if (pause_eval(30, 1000)) {
						...
						if (repl_buffer) {
							...
							lbm_create_string_char_channel(
								&string_tok_state, &string_tok, repl_buffer
							);
							repl_cid =
		...
		case COMM_LISP_STREAM_CODE: {
			...
			if (offset == 0) {
				if (string_tok_valid) {
					...
					if (timeout == 0) {
						...
						offset_last = -1;
						buffer_append_int16(
							send_buffer, result_last, &send_ind
						);
				...
				lbm_create_buffered_char_channel(
					&buffered_tok_state, &buffered_string_tok
				);
				string_tok_valid = true;

				if (lbm_load_and_eval_program(&buffered_string_tok, "main-s")
					<= 0) {
					lispif_unlock_lbm();
					result_last = -4;
					offset_last = -1;
			...
			while (ind < (int32_t)len) {
				int ch_res =
					lbm_channel_write(&buffered_string_tok, (char)data[ind]);
...
bool lispif_restart(bool print, bool load_code, bool load_imports) {
	...
	if (!load_code || (code_data != 0 && code_len > 0)) {
		...
		if (!lisp_thd_running) {
			lbm_init(
				heap, heap_size, memory_array, mem_size, bitmap_array,
				bitmap_size, GC_STACK_SIZE, PRINT_STACK_SIZE, extension_storage,
			...
			xTaskCreatePinnedToCore(
				eval_thread, "lbm_eval", 3072, NULL, 6, NULL, tskNO_AFFINITY
			);
		...
		// Load imports
		// Load imports (store IROM header address for native libs)
		if (load_imports && code_len > code_chars + 3) {
			int32_t ind = code_chars + 1;
			uint16_t num_imports =
				buffer_get_uint16((uint8_t *)code_data, &ind);
			...
			if (num_imports > 0 && num_imports < 500) {
				for (int i = 0; i < num_imports; i++) {
					...
					int32_t offset =
						buffer_get_int32((uint8_t *)code_data, &ind);
					...
					if (len > 12) {
						...
						if (magic_be == __builtin_bswap32(NATIVE_LIB_MAGIC)) {
							...
							commands_printf_lisp(
								"Stored import '%s' irom: 0x%08X", name,
								(unsigned)lbm_dec_as_u32(val)
							);
							lbm_define(name, val);
							handled = true;
						}
		...
		if ((((uint32_t)code_data + flash_helper_code_size_raw(CODE_IND_LISP)))
			> (uint32_t)const_heap_ptr) {
			uint32_t const_heap_len =
				((uint32_t)code_data
				 + flash_helper_code_size_raw(CODE_IND_LISP))
				- (uint32_t)const_heap_ptr;
			lbm_const_heap_init(
				const_heap_write, &const_heap, (lbm_uint *)const_heap_ptr,
				const_heap_len
			);
		}
		...
		if (load_code) {
			...
			lbm_create_string_char_channel(
				&string_tok_state, &string_tok, code_data
			);
...
static bool const_heap_write(lbm_uint ix, lbm_uint w) {
	...
	uint32_t offset = (uint32_t)const_heap_ptr
		- (uint32_t)flash_helper_code_data_raw(CODE_IND_LISP)
		+ sizeof(lbm_uint) * ix;
	flash_helper_write_code(
		CODE_IND_LISP, offset, (uint8_t *)&w, sizeof(lbm_uint),
		(const_heap_max_ind - ix) * sizeof(lbm_uint)
	);
