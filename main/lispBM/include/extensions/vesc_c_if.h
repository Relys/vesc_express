...
typedef struct {
	...
	// RGBLED (Express Only). todo need to break these out into helper functions in main fw #include "lispif_rgbled_extensions.h"
	//lbm_value rgbled_init(lbm_value *args, lbm_uint argn);
	//lbm_value rgbled_deinit(lbm_value *args, lbm_uint argn);
	//lbm_value rgbled_buffer(lbm_value *args, lbm_uint argn);
	//lbm_value rgbled_color(lbm_value *args, lbm_uint argn);
	//lbm_value rgbled_update(lbm_value *args, lbm_uint argn);
	...
	//need to look at make and split
} vesc_c_if;
