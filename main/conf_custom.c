...
commands_printf_lisp("custom config");
...
int conf_custom_get_cfg_xml(int conf_ind, uint8_t **data) {
	commands_printf_lisp("get xml");
	if (conf_ind > 2 || m_get_cfg_xml == 0) {
		return 0;
	}
...
void conf_custom_process_cmd(unsigned char *data, unsigned int len,
	...
	data++;
	len--;
commands_printf_lisp("process cmd");
	switch (packet_id) {
